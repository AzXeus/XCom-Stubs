...
function InitBaseSpawnMutator(int aNumSec)
{
	...
	if (NumSec == 0)
	{
		LogInternal("XComSpawnMutator: Error! Num sections mus be non-zero!");
	}
	...
	LogInternal("XComSpawnMutator: NumSec = " $ NumSec);
	LogInternal("XComSpawnMutator: PLAYER_PROXIMITY = " $ PLAYER_PROXIMITY);
	IsCaptureAndHoldMission = (XGBattle_SP(BATTLE()).m_kDesc.m_iMissionType == eMission_CaptureAndHold);
	if (IsCaptureAndHoldMission)
	{
		LogInternal("XComSpawnMutator: AVOID_CAPTURE_ZONES = " $ AVOID_CAPTURE_ZONES);
	}
	if (!InitPlayerStartLoc())
	{
		LogInternal("XComSpawnMutator: Can't find player start location, skipping to defaults.");
		return;
	}
	CheckOp = false;
	if (InitOpStartLoc())
	{
		...
		CheckOp = true;
		LogInternal("XComSpawnMutator: OP_PROXIMITY = " $ OP_PROXIMITY);
	}
...
function bool InitPlayerStartLoc()
{
	...
	foreach SquadSpawnPts(Pt)
	{
		...
		{
			LogInternal("XComSpawnMutator: Found SpawnPoint " $ Pt.Name $ " at " $ Pt.Location);
			AvPt += Pt.Location;
			++NumPts;
		}
	}
	if (NumPts == 0)
	{
		LogInternal("XComSpawnMutator: Error! No player spawn points found!");
		return false;
	}
	AvPt /= NumPts;
	LogInternal("XComSpawnMutator: Player Central Location " $ AvPt);
	PlayerStartLoc = AvPt;
	return true;
}
...
function bool InitOpStartLoc()
{
	   ...
	   XGBattle_SP(BATTLE()).m_kDesc.m_iMissionType == eMission_CaptureAndHold)
	{
		...
		LogInternal("XComSpawnMutator: Covert Operative start location = " $ OpStartLoc);
		return true;
	}
	return false;
}
...
function BuildGrid()
{
	...
	while (NumSec < 100)
	{
		...
		//NumSec += 2;
		++NumSec;
		LogInternal("XComSpawnMutator: Not enough sections left: " $ (Grid.Length - ArrRemove.Length) $ ", increasing NumSec to " $ NumSec);
	}
	...
	NumSec = Grid.Length;
	LogInternal("XComSpawnMutator: Final NumSec = " $ NumSec);
	/*foreach Grid(Section)
	{
...
function CalculateGrid()
{
	...
	LogInternal("XComSpawnMutator: WORLD().NumX = " $ WORLD().NumX);
	LogInternal("XComSpawnMutator: WORLD().NumY = " $ WORLD().NumY);
	...
	LogInternal("XComSpawnMutator: NumSecX = " $ NumSecX);
	LogInternal("XComSpawnMutator: NumSecY = " $ NumSecY);
	...
	LogInternal("XComSpawnMutator: BeginX = " $ BeginX);
	LogInternal("XComSpawnMutator: BeginY = " $ BeginY);
	LogInternal("XComSpawnMutator: StepX = " $ StepX);
	LogInternal("XComSpawnMutator: StepY = " $ StepY);
...
function array<vector> GetRandomLocations(optional bool bRandomZ = true)
{
	...
	foreach Grid(Section)
	{
		...
		if (BadLoc)
		{
			...
			LogInternal("XComSpawnMutator: Failed to find random location, using section center!");
		}
		RandLoc.AddItem(SpawnLoc);
	}
	LogInternal("XComSpawnMutator: Found " $ RandLoc.Length $ " random locations.");
	return RandLoc;
}
