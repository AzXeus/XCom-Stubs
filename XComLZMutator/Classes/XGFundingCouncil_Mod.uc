...
function Init()
{
	...
	FilterVanillaMaps(MapNames);
	LogInternal("XGFundingCouncil_Mod: extended special maps array length = " $ MapNames.Length);
	if (MapNames.Length > 0)
	{
		...
		for (i = 0; i < MapNames.Length; ++i)
		{
			...
			LogInternal("XGFundingCouncil_Mod: m_arrTMissions[" $ idx $ "].eMission = " $ tmp);
		}
	}
	LogInternal("XGFundingCouncil_Mod: m_arrTMissions.Length = " $ m_arrTMissions.Length);
}
...
function EFCMission ChooseNextMissionByType(EMissionRegion eRegion, ECountry fcCountry)
{
	...
	{
		LogInternal("XGFundingCouncil_Mod: all missions taken, clearing extended maps play history.");
