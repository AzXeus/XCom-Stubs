...
{
	LogInternal("XComMeldSpawnMutator: PostLevelLoaded");
	// to always have 1 easy and 1 hard container
	if (NumContainers < 2)
	{
		NumContainers = 2;
	}
	LogInternal("XComMeldSpawnMutator: NumContainers = " $ NumContainers);
	if (EqSlope == 0)
	{
		EqSlope = 1;
	}
	...
	{
		LogInternal("XComMeldSpawnMutator: no need to spawn Meld, skipping.");
		return;
	}
	...
	{
		LogInternal("XComMeldSpawnMutator: Error, can't find enough spawn locations, skipping!");
		return;
	}
