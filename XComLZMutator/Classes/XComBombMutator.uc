...
function MutateUpdateInteractClaim(string UnitObjName, PlayerController Sender)
{
	...
	LogInternal("XComBombMutator: UnitObjName = " $ UnitObjName);
	...
	foreach AllActors(class'XGUnit', Unit)
	{
		...
		{
			FoundUnit = Unit;
			LogInternal("XComBombMutator: FoundUnit = " $ FoundUnit);
			break;
		}
	}
...
function FixUpdateInteractClaim(XGUnit Unit)
{
	...
	LogInternal("Unit.m_arrInteractPoints.Length = " $ Unit.m_arrInteractPoints.Length);
}
...
function DoWorldDataRebuild(PlayerController Sender)
{
	...
	LogInternal("XComBombMutator: DoWorldDataRebuild");
	if (Active)
	{
		LogInternal("XComBombMutator: restoring from save");
		Loc = TheBomb.Loc;
		MoveBombObjects(Loc);
		for (I = 0; I < ArrNodes.Length; ++I)
		{
...
function PostLevelLoaded(PlayerController Sender)
{
	...
	LogInternal("XComBombMutator: PostLevelLoaded");
	
	if (MapData.MissionType == eMission_Special && MapData.CouncilType == eFCMType_Bomb)
	{
		LogInternal("XComBombMutator: enabled");
	}
	else
	{
		LogInternal("XComBombMutator: disabled");
		return;
	}
	...
	if (!FindBomb())
	{
		LogInternal("XComBombMutator: can't find the bomb, skipping to defaults");
		return;
	}
	if (!FindNodes())
	{
		LogInternal("XComBombMutator: can't find power nodes, skipping to defaults");
		return;
	}
	LogInternal("XComBombMutator: found " $ string(ArrNodes.Length) $ " nodes");
	
	InitBaseSpawnMutator(ArrNodes.Length + 1);
	ArrNewLoc = GetRandomLocations(false);
	if (ArrNewLoc.Length < ArrNodes.Length + 1)
	{
		LogInternal("XComBombMutator: can't find enough spawn locations, skipping to defaults");
		return;
	}
...
function ModifyKismet()
{
	...
	LogInternal("XComBombMutator: Checking MainSequence");
	...
	for (J = 0; J < Objects.Length; ++J)
	{
		...
		{
			LogInternal("XComBombMutator: disabling BombTutorial sequence");
	...
	if (TimerVariable != none)
	{
		if (INITIAL_TIMER_VALUE > 0)
		{
			...
			LogInternal("XComBombMutator: initial timer value set to " $ string(TimerVariable.IntValue));
		}
	}
}
...
function bool FindBomb()
{
	...
	LogInternal("XComBombMutator: locating the bomb");
	...
	foreach AllActors(class'Actor', A)
	{
		...
		{
			LogInternal("Found actor: " $ A.Name);
...
function MoveBomb()
{
	...
	LogInternal("XComBombMutator: moving the bomb");
...
function MoveBombObjects(vector Loc)
{
	...
	LogInternal("XComBombMutator: Bomb " $ TheBomb.Bomb.Name $ " Location " $ TheBomb.Bomb.Location);
	//TheBomb.Bomb.SnapToGround();
	TheBomb.Loc = TheBomb.Bomb.Location;
	TheBomb.Button.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: Button " $ TheBomb.Button.Name $ " Location " $ TheBomb.Button.Location);
	TheBomb.WaypointFloor.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: WaypointFloor " $ TheBomb.WaypointFloor.Name $ " Location " $ TheBomb.WaypointFloor.Location);
	TheBomb.Destroyed.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: Destroyed " $ TheBomb.Destroyed.Name $ " Location " $ TheBomb.Destroyed.Location);
	TheBomb.Active.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: Active " $ TheBomb.Active.Name $ " Location " $ TheBomb.Active.Location);
	TheBomb.Inactive.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: Inactive " $ TheBomb.Inactive.Name $ " Location " $ TheBomb.Inactive.Location);
	TheBomb.Exploding.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: Exploding " $ TheBomb.Exploding.Name $ " Location " $ TheBomb.Exploding.Location);
	TheBomb.RebuildVolume.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: RebuildVolume " $ TheBomb.RebuildVolume.Name $ " Location " $ TheBomb.RebuildVolume.Location);
	TheBomb.VisiblePoint.SetLocation(TheBomb.Loc);
	LogInternal("XComBombMutator: VisiblePoint " $ TheBomb.VisiblePoint.Name $ " Location " $ TheBomb.VisiblePoint.Location);
	for (I = 0; I < TheBomb.ArrPoints.Length; ++I)
	{
		TheBomb.ArrPoints[I].SetLocation(TheBomb.Loc + vect(0,0,206));
		LogInternal("XComBombMutator: ArrPoints[" $ I $ "] " $ TheBomb.ArrPoints[I].Name $ " Location " $ TheBomb.ArrPoints[I].Location);
	}
...
function DestroyFacingWaypoint()
{
	...
	foreach AllActors(class'Actor', A)
	{
		if (A.Class == Class'Emitter' && A.ObjectArchetype.Name == 'ARC_facingWaypoint')
		{
			LogInternal("Found FacingWaypoint actor: " $ A.Name);
			TheBomb.FacingWaypoint = Emitter(A);
...
function bool FindNodes()
{
	...
	LogInternal("XComBombMutator: locating nodes");
...
function MoveNodes()
{
	...
	LogInternal("XComBombMutator: moving nodes");
...
function MoveNodeObjects(int I, vector Loc)
{
	...
	LogInternal("XComBombMutator: Node " $ ArrNodes[I].Node.Name $ " Location " $ ArrNodes[I].Node.Location);
	//ArrNodes[I].Node.SnapToGround();
	ArrNodes[I].Loc = ArrNodes[I].Node.Location;
	ArrNodes[I].Button.SetLocation(ArrNodes[I].Loc);
	LogInternal("XComBombMutator: Button " $ ArrNodes[I].Button.Name $ " Location " $ ArrNodes[I].Button.Location);
	ArrNodes[I].Inactive.SetLocation(ArrNodes[I].Loc);
	LogInternal("XComBombMutator: Inactive " $ ArrNodes[I].Inactive.Name $ " Location " $ ArrNodes[I].Inactive.Location);
	ArrNodes[I].Active.SetLocation(ArrNodes[I].Loc);
	LogInternal("XComBombMutator: Active " $ ArrNodes[I].Active.Name $ " Location " $ ArrNodes[I].Active.Location);
