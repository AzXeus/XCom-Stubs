...
function MutateNotifyKismetOfLoad(PlayerController Sender)
{
	...
	LogInternal("XComLZMutator: Checking MainSequence");
	...
	for (J = 0; J < Variables.Length; ++J)
	{
		...
		{
			LogInternal("XComLZMutator: found SpawnGroupIndex");
	...
	if (DisableKismetHideLZ)
	{
		...
		for (I = 0; I < Events.Length; ++I)
		{
			...
			if (Objects.Length != 0)
			{
				LogInternal("XComLZMutator: found SeqAct_ToggleHidden: " $ Events[I].Name);
...
{
	LogInternal("XComLZMutator: PostLoadSaveGame");
	LogInternal("XComLZMutator: saved idx = " $ IdxSaved);
	if (IdxSaved != -1)
	{
		LogInternal("XComLZMutator: Re-spawning LZ using saved idx");
		/// re-spawn saved LZ
...
function PostLevelLoaded(PlayerController Sender)
{
	...
	LogInternal("XComLZMutator: LZArray.Length = " $ LZArray.Length);
	...
	LogInternal("XComLZMutator: CurMapName = " $ CurMapName);
	MapData = class'XComMapManager'.static.GetCurrentMapMetaData();
	
	if (Battle == None)
	{
		LogInternal("XComLZMutator: Error! Battle == None!");
	}
	...
	{
		LogInternal("XComLZMutator: Disabled for MissionType " $ MapData.MissionType $ ", CouncilType " $ MapData.CouncilType);
	}
	else
	{
		if (MapData.CouncilType == eFCMType_Bomb)
		{
			...
			while (FindMutator != none)
			{
				...
				{
					LogInternal("XComLZMutator: XComBombMutator is active, enabling LZ randomization for Bomb Defusal missions.");
					break;
				}
		...
		{
			LogInternal("XComLZMutator: Data.MapName = " $ Data.MapName);
			...
			if ( CurMapName ~= Data.MapName )
			{
				...
				if ( Data.MissionType == eFCMType_None || MapData.MissionType == Data.MissionType )
				{
					...
					{
						LogInternal("XComLZMutator: config entry matches current map, adding");
		...
		{
			LogInternal("XComLZMutator: DataArray.Length = " $ DataArray.Length);
			/// restore saved LZ on loading from save
			if (IdxSaved >= 0 && IdxSaved < DataArray.Length)
			{
				LogInternal("XComLZMutator: using saved idx");
				idx = IdxSaved;
			}
			/// choose random LZ
			else
			{
				LogInternal("XComLZMutator: using random idx");
				idx = Rand(DataArray.Length);
				IdxSaved = idx;
			}
			LogInternal("XComLZMutator: #" $ idx);
			Data = DataArray[idx];
			HideDropShip();
			if (Data.ESpawnLoc == ESL_Default)
			{
				LogInternal("XComLZMutator: using default map LZ");
			}
			else
			{
				if (!FindLZObjectsByArchetype(Data))
				{
					LogInternal("XComLZMutator: Error! Can't find LZ objects!");
				}
				else
				{
		...
		{
			LogInternal("XComLZMutator Test: SpawnPoint " $ Pt.Name $ " at " $ Pt.Location);
		}
	}
}
...
function HideDropShip()
{
	...
	foreach AllActors(class'Actor', A)
	{
		...
		{
			LogInternal("Test find prefabs: SoldierSpawns_DrpshipVol arc found: " $ string(A.Name));
			A.SetHidden(true);
			A.Destroy();
		}
		...
		{
			LogInternal("Test find prefabs: SkyrangerGrounded arc found: " $ string(A.Name));
			A.SetHidden(true);
			A.Destroy();
		}
		...
		{
			LogInternal("Test find prefabs: Skyranger_Ground_Periphery arc found: " $ string(A.Name));
			Emitter(A).ShutDown();
			A.SetHidden(true);
			A.Destroy();
		}
...
function bool FindLZObjectsByArchetype(LZData Data)
{
	...
	foreach AllActors(class'Actor', A)
	{
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_12Arc0')
		{
			if (LZObjects.LevelActorObj == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_12Arc0 found: " $ string(A.Name));
				LZObjects.LevelActorObj = A;
				++EvacObjCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_12Arc0 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc0')
		{
			if (LZObjects.BuildingVolumeObj == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc0 found: " $ string(A.Name));
				LZObjects.BuildingVolumeObj = A;
				++EvacObjCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc0 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_2Arc0')
		{
			if (LZObjects.PlayerStartObj == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_2Arc0 found: " $ string(A.Name));
				LZObjects.PlayerStartObj = A;
				++OtherObjCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_2Arc0 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_12Arc1')
		{
			if (LZObjects.PointObj == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_12Arc1 found: " $ string(A.Name));
				LZObjects.PointObj = A;
				++OtherObjCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_12Arc1 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc1')
		{
			if (LZObjects.SpawnPt1 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc1 found: " $ string(A.Name));
				LZObjects.SpawnPt1 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc1 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc2')
		{
			if (LZObjects.SpawnPt2 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc2 found: " $ string(A.Name));
				LZObjects.SpawnPt2 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc2 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc3')
		{
			if (LZObjects.SpawnPt3 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc3 found: " $ string(A.Name));
				LZObjects.SpawnPt3 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc3 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc4')
		{
			if (LZObjects.SpawnPt4 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc4 found: " $ string(A.Name));
				LZObjects.SpawnPt4 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc4 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_Arc5')
		{
			if (LZObjects.SpawnPt5 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc5 found: " $ string(A.Name));
				LZObjects.SpawnPt5 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_Arc5 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
		if (A.ObjectArchetype.Name == 'SoldierSpawns_DrpshipVol_7Arc0')
		{
			if (LZObjects.SpawnPt6 == None)
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_7Arc0 found: " $ string(A.Name));
				LZObjects.SpawnPt6 = A;
				++SpawnPointsCount;
			}
			else
			{
				LogInternal("Find LZ objects: SoldierSpawns_DrpshipVol_7Arc0 found duplicate: " $ string(A.Name));
				A.SetHidden(true);
				A.Destroy();
			}
		}
...
function CalcLocations(LZData Data)
{
	...
	LogInternal("XComLZMutator: Spawn Point 1 Location: " $ string(sp1Loc));
	LogInternal("XComLZMutator: Spawn Point 2 Location: " $ string(sp2Loc));
	LogInternal("XComLZMutator: Spawn Point 3 Location: " $ string(sp3Loc));
	LogInternal("XComLZMutator: Spawn Point 4 Location: " $ string(sp4Loc));
	LogInternal("XComLZMutator: Spawn Point 5 Location: " $ string(sp5Loc));
	LogInternal("XComLZMutator: Spawn Point 6 Location: " $ string(sp6Loc));
}
...
defaultproperties
{
    dps=(X=8.0,Y=189.0,Z=16.0)
    dbv=(X=0.0,Y=-47.0,Z=32.0)
    dla=(X=0.0,Y=0.0,Z=-64.0)
    dsp1=(X=192.0,Y=384.0,Z=-16.0)
    dsp2=(X=0.0,Y=480.0,Z=-16.0)
    dsp3=(X=192.0,Y=192.0,Z=-16.0)
    dsp4=(X=0.0,Y=288.0,Z=-16.0)
    dsp5=(X=-192.0,Y=384.0,Z=-16.0)
    dsp6=(X=-192.0,Y=192.0,Z=-16.0)
    DP=(X=-240.0,Y=432.0,Z=-64.0)
    IdxSaved=-1
}
