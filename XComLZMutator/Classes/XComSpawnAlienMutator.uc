...
function bool IsMutatorValid(string SeqActObjName)
{
	...
	foreach SequenceObjects(Obj)
	{
		...
		{
			LogInternal("XComSpawnAlienMutator: found obj " $ Obj.Name);
			SpawnAlienObj = SeqAct_SpawnAlien(Obj);
			++SpawnAlienObjCount;
		}
	}
	LogInternal("XComSpawnAlienMutator: SeqActObjName = " $ SeqActObjName);
	
	CurMapName = class'Engine'.static.GetCurrentWorldInfo().GetMapName();

	if (XGBattle_SP(BATTLE()).m_kDesc.m_iMissionType == eMission_Final)
	{
		LogInternal("XComSpawnAlienMutator: Final mission is on, skipping to defaults.");
		return false;
	}
	else if (ExcludeMaps.Find(CurMapName) != -1)
	{
		LogInternal("XComSpawnAlienMutator: current map(" $ CurMapName $ ") found in ExcludeMaps list, skipping to defaults.");
		return false;
	}
	else if (SpawnAlienObjCount == 0)
	{
		LogInternal("XComSpawnAlienMutator: Error! Can't find " $ SeqActObjName);
		return false;
	}
	else if (SpawnAlienObjCount > 1)
	{
		LogInternal("XComSpawnAlienMutator: Error! Multiple " $ SeqActObjName $ " objects found!");
		return false;
	}
	return true;
}
...
function MutateSpawnAlien(string SeqActObjName, PlayerController Sender)
{
	...
	LogInternal("XComSpawnAlienMutator: MutateSpawnAlien");
	LogInternal("XComSpawnAlienMutator: MIN_DROP_DIST = " $ MIN_DROP_DIST);
	LogInternal("XComSpawnAlienMutator: MAX_DROP_DIST = " $ MAX_DROP_DIST);
	...
	if (IsMutatorValid(SeqActObjName) == true)
	{
		...
		{
			LogInternal("XComSpawnAlienMutator: Can't find player location, skipping to defaults.");
			return;
		}
		if (GetRandomSpawnLoc() == false)
		{
			LogInternal("XComSpawnAlienMutator: Can't find good spawn point, skipping to defaults.");
			return;
		}
...
function bool GetUnitsLoc()
{
	...
	while (kSoldier != none)
	{
		LogInternal("XComSpawnAlienMutator: Found good unit " $ kSoldier.Name);
		UnitsLoc.AddItem(kSoldier.Location);
        kSoldier = kSquad.GetNextGoodMember(kSoldier,, false);
	}
	if (UnitsLoc.Length < 1)
	{
		LogInternal("XComSpawnAlienMutator: Error! No good units found!");
		return false;
	}
	return true;
}
...
function GetCoverPoints()
{
	...
	//{
	    ...
	    foreach kCoverPts(kCover)
        {
			...
			LogInternal("XComSpawnAlienMutator: Found good cover point at " $ string(kCover.CoverLocation));
		}
	//}
}
...
function bool GetRandomSpawnLoc()
{
	...
	if (DROP_TO_COVER == true)
	{
		...
		if (CoverPoints.Length > 0)
		{
			...
			LogInternal("XComSpawnAlienMutator: SpawnLoc = " $ SpawnLoc);
			return true;
		}
	}
	...
	while (I < 100)
	{
		...
		LogInternal("XComSpawnAlienMutator: TestLoc = " $ TestLoc);
